...
export const raidSessionRoster = pgTable('raid_sessions_roster', {
  id: varchar('id').primaryKey(),
...
export const assignment = pgTable('assignments', {
  id: varchar('id').primaryKey(),
  charId: varchar("char_id").references(() => char.id),
  lootId: varchar('loot_id').references(() => loot.id).unique('loot_assignment'), // un loot può essere assegnato una sola volta
});
